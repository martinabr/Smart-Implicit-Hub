...
void setup() {
  ...
  mdns.begin(WiFi.localIP(), "Pixel2");

  char a1[] = "sensor1=/Pressure2:0%1000";
  char a2[] = "actuator1=/Motor2:0%1000";
  ...
  int success;
  success = mdns.addServiceRecord("Pixel 2._osc", 3333,
      MDNSServiceUDP, txt);
...
void loop() {
 ...
 if (client) {                             // if you get a client,
  ...
  if (server_ready == true) {
    ...
    //// Sending ////
    OSCMessage send_msg_pressure("/Pressure2");
  ...
  if (newPressure != oldPressure) {
    ...
    PWM = map(newPressure, 300, 1000, 0, 255);   // speed for retraction
    Inverse_PWM = map(newPressure, 1000, 300, 0, 255);   // speed for extraction
    level = map(newPressure, 300, 1000, 12550, 0);
  }
  ...
  //// Recieving ////
    OSCMessage recieve_msg("/Pressure2");
    ...
    if (udp_osc.remoteIP() == server_ip) {
      if (size > 0) {
        ...
        if (!recieve_msg.hasError()) {
          recieve_msg.dispatch("/Motor2", motor);
